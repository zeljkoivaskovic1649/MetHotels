...
public class FacebookService {
    ...
    private final String clientId = "1669003136648251";
    private final String clientSecret = "55838a0020d52871cc7140a0b5755e11";
    ...
    public void getUserAccessToken(String code, String token) throws IOException, OAuthSystemException, OAuthProblemException {
        OAuthClientRequest request = OAuthClientRequest.tokenLocation("https://graph.facebook.com/oauth/access_token")
        .setGrantType(GrantType.AUTHORIZATION_CODE).setClientId(clientId).setClientSecret(clientSecret)
        .setRedirectURI(getCallBackUrl()).setCode(code).setParameter("scope", SCOPE).buildBodyMessage();
        OAuthClient oAuthClient = new OAuthClient(new URLConnectionClient());
        OAuthAccessTokenResponse oAuthResponse = oAuthClient.accessToken(request, GitHubTokenResponse.class);
    ...
    public String getFacebookAuthentificationLink() throws OAuthSystemException {
        return OAuthClientRequest.authorizationLocation("https://www.facebook.com/dialog/oauth/?")
        .setClientId(clientId).setRedirectURI(getCallBackUrl()).setScope(SCOPE).buildQueryMessage().getLocationUri();
    }
